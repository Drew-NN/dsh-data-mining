...
import { buildProfile, DEFAULT_MAX_BYTES, parseCsv, readCsvFile, selectRows, valueCounts } from "./profile.js";
import { discoverDataFiles } from "./discover.js";
import { splitDatasetFile, checkLeakageFile, readSplitMetadata } from "./split.js";
import { MANIFEST_FILENAME, addDataset, loadManifest, recordDecision, saveManifest, setGoal, setPhase, setSplitRef, } from "./manifest.js";
/** Cordis plugin name (preset entry; distinct from the global entry). */
export const name = 'data-mining-tools';
...
export function apply(ctx) {
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (!args.path)
                throw new Error('profile_dataset: `path` must be a non-empty string');
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (!args.path)
                throw new Error('sample_rows: `path` must be a non-empty string');
            ...
            if (args.columns !== undefined)
                selection.columns = args.columns;
            if (args.where !== undefined && args.where !== null)
                selection.where = args.where;
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (!args.path)
                throw new Error('value_counts: `path` must be a non-empty string');
            if (!args.column)
                throw new Error('value_counts: `column` must be a non-empty string');
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (!args.path)
                throw new Error('split_dataset: `path` must be a non-empty string');
            if (!args.name)
                throw new Error('split_dataset: `name` must be a non-empty string');
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            if (!args.splitFile)
                throw new Error('check_leakage: `splitFile` must be a non-empty string');
            const maxBytes = args.maxBytes === undefined ? DEFAULT_MAX_BYTES : Math.max(1, args.maxBytes);
        ...
        async execute(args, exec) {
            ...
            switch (args.action) {
                ...
                case 'set_goal': {
                    ...
                    if (next.phase === null)
                        next = setPhase(next, 'business');
                    break;
                }
                case 'set_phase': {
                    if (args.phase === undefined)
                        throw new Error('manifest set_phase: `phase` is required');
                    next = setPhase(current, args.phase);
                    break;
                }
                case 'add_dataset': {
                    if (!args.path)
                        throw new Error('manifest add_dataset: `path` is required');
                ...
                case 'record_decision': {
                    if (!args.text)
                        throw new Error('manifest record_decision: `text` is required');
                    next = recordDecision(current, args.text);
                    break;
                }
                case 'set_split': {
                    if (!args.splitFile)
                        throw new Error('manifest set_split: `splitFile` is required');
