...
function toRow(line, headers, delimiter) {
...
export function isDatetimeValue(value) {
    ...
    if (!DATETIME_RE.test(trimmed) && !SLASH_DATE_RE.test(trimmed))
        return false;
...
export function inferKind(values) {
    ...
    if ((hasNumber ? 1 : 0) + (hasBoolean ? 1 : 0) + (hasDatetime ? 1 : 0) > 1)
        return 'string';
    if (hasNumber)
        return 'number';
    if (hasDatetime)
 ...
 */
export function profileColumn(name, values, maxSample) {
 ...
 */
export function buildProfile(table, path, bytes, maxSample) {
    const columns = [];
 ...
 */
export function valueCounts(table, columnIndex, topK) {
    ...
    const values = entries.slice(0, topK).map(([value, count]) => ({
        value,
        count,
 ...
 */
export function selectRows(table, selection, offset, limit) {
    ...
    for (const source of table.rows) {
        if (whereIdx !== -1) {
            ...
            if (cell === null || cell !== selection.where.equals)
                continue;
        }
 ...
 */
export async function readCsvFile(path, signal, opts = {}) {
