...
- 计数基于**已读入（可能已采样）的行**，`sampled`/`truncated` 继续说明近似性。
- 缺失值单独统计（不混进 top-k），方便识别"哨兵值冒充数据"的场景。

**验收**：
- [ ] 已知数据手算计数、占比、排序（含并列值顺序）
- [ ] topK 截断 + omitted 数量正确
- [ ] 列名不存在时报错
- [ ] 缺失单独统计；采样时 sampled 标记
...
- 输出：`{ root, fileCount, files: [{ path, ext, kind, bytes, delimiter?, rowEstimate?, estimated }] }`，按路径排序。

**验收**：
- [ ] 递归找到数据文件、大小正确
- [ ] csv 分隔符嗅探正确（tab/逗号）
- [ ] 行数估算：小文件精确、大文件外推且标记 estimated
- [ ] 隐藏目录与 node_modules 被跳过；超过 maxFiles 截断
...
- 输出加 `columns` 回声与 `totalMatches`（无 where 时等于 totalRows）。

**验收**：
- [ ] 列过滤：只返回指定列、顺序正确、未知列报错
- [ ] where 精确匹配 + offset/limit 作用于匹配集 + totalMatches 正确
- [ ] 既有行为不回归（无 columns/where 时与之前一致）
