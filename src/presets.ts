...
export function userPresetRoot(dshHome = process.env.DSH_HOME): string {
 ...
 */
export async function ensurePresetsInstalled(dshHome?: string): Promise<string[]> {
  ...
  for (const id of SHIPPED_PRESETS) {
    ...
    if (existsSync(target)) {
      ...
      if (existing === fresh || !existing.includes(LEGACY_ENTRY_MARKER)) continue
      // legacy row: replace
    }
