...
import { detectDelimiter } from './profile.ts'
...
const EXT_KIND: Record<string, DataKind> = {
  csv: 'csv',
  tsv: 'csv',
  json: 'json',
  jsonl: 'json',
 ...
 */
async function estimateCsv(path: string, signal: AbortSignal): Promise<{ delimiter: string; rowEstimate: number; estimated: boolean }> {
...
async function walk(root: string, opts: { maxDepth: number; maxFiles: number; signal: AbortSignal }, files: DataFileInfo[], depth: number): Promise<boolean> {
 ...
 */
export async function discoverDataFiles(root: string, opts: DiscoverOptions = {}): Promise<DiscoveryResult> {
  ...
  if (!st.isDirectory()) throw new Error(`discoverDataFiles: "${root}" is not a directory`)
