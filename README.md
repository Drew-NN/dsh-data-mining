...
| **Data tools** (`discover_datasets`, `profile_dataset`, `value_counts`, `sample_rows`, `split_dataset`, `check_leakage`) | Give the model a proxy view of datasets that cannot fit in its context window — workspace inventory, schema, missing rates, distribution stats, value frequencies, row access — plus mechanical split/leakage guardrails (deterministic train/test splits with recorded metadata; verification that no test information reached training). Robust CSV parsing (BOM, delimiter detection), byte/row caps, and honest `sampled`/`truncated` flags |
